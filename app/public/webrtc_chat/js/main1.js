...
function getMyMedia(){
  ...
  navigator.mediaDevices.getUserMedia(mediaConstraints)
    .then(str => {
      setLocalStream(str);//set local stream  
      doSignal();//start signaling
    })
    .catch(err => { 
      // console.log('Could not get Media: ', err);
      alert('Vui lòng cấp quyền camera và mic để có thể sử dụng'); 
    });
}
...
function doSignal() {
  ...
  sig.on('message', msg => {
    ...
    var payload = pkt.p;//du lieu duoc gui
    //meta object-> f:MyFirstApp/a, o: status peer,and t: to
    //f: from,tu ai do gui cho ban
    //t: to,nguoi nhan
    var meta = pkt.m;
...
// them giao dien cho trang index
